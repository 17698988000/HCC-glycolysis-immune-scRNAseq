...
# Configure LIANA_SEURAT_RDS to point to the local LIANA input object.
...
if (length(missing_pkgs) > 0) {
  stop("Missing package(s): ", paste(missing_pkgs, collapse = ", "),
       ". Install dependencies before running; this script does not install packages at runtime.")
}
for (p in pkgs) suppressPackageStartupMessages(library(p, character.only = TRUE))

set.seed(20260528)

OBJ_RDS      <- Sys.getenv("LIANA_SEURAT_RDS", unset = file.path(project_dir, "hcc_seurat.rds"))
...
cat("LIANA directional-concordance analysis\n")
