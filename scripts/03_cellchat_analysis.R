...
#   - no setwd("machine-specific local path")
...
#   n = 15,391
#   median Glycolysis_AUC cutoff = 0.212617779598525
#   GlycoHigh/GlycoLow = locked rank-balanced Figure 2C assignment, n = 7,695 / 7,696
#   
#
# CellChat rounds:
...
locked_assignments <- load_locked_fig2c_assignments()
locked_map <- stats::setNames(locked_assignments$GlycoGroup, locked_assignments$cell)
tumor_hepa <- rownames(meta) %in% locked_assignments$cell
tumor_hepa_n <- sum(tumor_hepa)
median_cut <- LOCKED_FIG2C_MEDIAN_AUC

glyco_group <- rep(NA_character_, nrow(meta))
matched_locked_groups <- unname(locked_map[rownames(meta)[tumor_hepa]])
glyco_group[tumor_hepa] <- ifelse(matched_locked_groups == "GlycoHigh", "TumorGlycoHigh", "TumorGlycoLow")
if (anyNA(glyco_group[tumor_hepa])) stop("Locked Figure 2C grouping could not be mapped to every CellChat tumor-derived hepatocyte.", call. = FALSE)
