# ============================================================
# 12_GSE235863_exploratory_association.R
#
# Current purpose:
#   Exploratory GSE235863 anti-PD-1 plus lenvatinib association
#   analysis using the locked TCGA-derived TPI1/ENO1/LDHA/SLC2A1
#   four-gene tissue-level readout.
#
# Manuscript boundary:
#   This script supports descriptive, hypothesis-generating summaries only.
#   It does not perform ROC/PR analysis, DCA, machine learning, drug
#   repurposing, response prediction, or treatment-selection modelling.
#
...
#   Expected descriptive result:
#     Median-based stratification with the median sample included in
#     the High group places all four non-responders in the High group.
#     Fisher exact p approximately 0.077.
#
# Mandatory output order:
#   1. Source CSV files
#   2. QC CSV file
#   3. Descriptive PDF/PNG figures only if QC PASS
#
# Interpretation:
#   Hypothesis-generating / exploratory association only.
#   Not a response predictor.
#   Not a treatment-selection model.
# ============================================================
...
# ----------------------------
# The current manuscript uses only descriptive comparisons for GSE235863.
# No ROC, PR, DCA, complex ML, or treatment-selection analysis is performed.
wilcox_p <- stats::wilcox.test(
  four_gene_score ~ response_class,
  data = source_df,
  exact = FALSE
)$p.value
...
manifest <- tibble::tibble(
  ...
  value = c(
    ...
    "Hypothesis-generating exploratory association only; not a treatment-selection model."
  )
)
...
qc <- dplyr::bind_rows(
  ...
  make_qc_row("Wilcoxon descriptive p-value", sprintf("%.6f", wilcox_p),
              "Descriptive only; no deployment threshold", TRUE),
...
message("Completed 12_GSE235863_exploratory_association.R descriptive analysis.")
